...
unique(compadre$SpeciesAccepted[compadre$OrganismType=="Palm"])

# Extract metadata from a particular species, e.g.: the Euterpe palm 
eut_prec_meta <- subset(compadre, compadre$SpeciesAccepted == "Euterpe precatoria")

# Extract matrices from this species
eut_prec_mat <- subset(compadre$mat, compadre$SpeciesAccepted == "Euterpe precatoria")

# Check for availablity of matrices from another species, e.g.: African cherry
prun_afr_meta <- subset(compadre, SpeciesAccepted == "Prunus africana")
prun_afr_mat <- subset(compadre$mat, compadre$SpeciesAccepted == "Prunus africana")
...
deer_dd
#####################################
# Step 10. Stochastic model acai    #
#####################################

# Load the matrix for a normal and dry year
N  <- as.matrix(read.table(file = "Euterpe_matrix.csv", sep=",", header = FALSE))
D  <- as.matrix(read.table(file = "Euterpe_matrix_dry.csv", sep=",", header = FALSE))
n0  <- as.matrix(read.table(file = "Euterpe_vector.csv", sep=",", header = FALSE))

# List the matrices to use
list_mat <- list(N,D)

# Conduct the stochastic simulations, with 0.12 probability for a Dry year
# This is the result for a single simulation; the function gives the final number number of indiviuals 
eut_stoch_1_sim <- stoch.projection(list_mat, n0,tmax=50,nrep=1,prob=c(0.88,0.12))
# Population size is here:
rowSums(eut_stoch_1_sim)


plot(t(eut_stoch_1_sim))
# This is the result for 10 simulations; the function gives the final number number of indiviuals 
eut_stoch_10_sim <- stoch.projection(list_mat, n0,tmax=50,nrep=10,prob=c(0.88,0.12))
rowSums(eut_stoch_10_sim)

# Now for many simulations  
eut_stoch_1000_sim <- stoch.projection(list_mat, n0,tmax=50,nrep=1000,prob=c(0.88,0.12))
rowSums(eut_stoch_1000_sim)
hist(rowSums(eut_stoch_1000_sim))

# We can calculate the stochastic population growth rate  
# This yields two approximations of log(population growth)
stoch_r<-stoch.growth.rate(list_mat, prob = c(0.88,0.12), maxt = 100, verbose = TRUE)
